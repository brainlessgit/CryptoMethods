...
public class Interface {
    ...
    public Interface(BigInteger n, boolean cert) {
        //com.kpi.aks.MillerRabin.verbose = com.kpi.aks.Interface.verbose;
        //com.kpi.aks.AKS.verbose = com.kpi.aks.Interface.verbose;
    ...
    public boolean isPrime() {
        if (this.toTest.compareTo(AKSBOUND) < 0) {
            VerbosePrint("n is small, running com.kpi.aks.AKS only");
            if (m_printcert)
                AKS.verbose = true;
            AKSThread = new AKS(this.toTest);
            m_isprime = AKSThread.isPrime();
            VerbosePrint("com.kpi.aks.AKS says " + m_isprime);
  ...
  private BigInteger BigIntSqrt()
  {
    ...
    private boolean StartThreads() {
        AKSThread = new AKS(this.toTest);
        if (verbose)
            System.out.println("Starting com.kpi.aks.AKS thread with: " + this.toTest.toString());
        AKSThread.start();
        ...
        while (true) {
            if (!AKSThread.isAlive() && !aksaliveprint) {
                VerbosePrint("com.kpi.aks.AKS says " + this.toTest.toString() + " is " + AKSThread.n_isprime);
